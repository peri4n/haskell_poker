...
data Rank
  = HighCard [Value] -- All 5 cards
  | Pair Value
         Cards -- The pair and 3 kickers
  | TwoPair Value
            Value
            Card -- Two pairs a kicker
  | ThreeOfAKind Value
                 Cards -- The tripple and two kickers
  | Straight Value -- High card of the straigt
  | Flush Value -- High card of the flush
  | FullHouse Value
              Value -- Tripples and pair
  | FourOfAKind Value
                Value -- Quadruple and a kicker
...
instance Show Rank where
  ...
  show (TwoPair p1 p2 x) =
    "Pair of " ++ show p1 ++ " and " ++ show p2 ++ "with kicker " ++ show x
